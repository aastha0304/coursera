...
object Huffman {
  ...
  case class Leaf(char: Char, weight: Int) extends CodeTree {
    override def contains(head: Char): Boolean = char == head
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def inner(innerTree: CodeTree, bits: List[Bit]): List[Char] = innerTree match {
      case Leaf(char, _) => if (bits.isEmpty) List(char) else char :: inner(tree, bits)
      case Fork(left, right, _, _) => if (bits.head ==  0) inner(left, bits.tail) else inner(right, bits.tail)
    }
    inner(tree, bits)
  }
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = if(table.isEmpty) throw new NoSuchElementException else if(table.head._1 == char) table.head._2 else codeBits(table.tail)(char)
