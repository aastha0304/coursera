...
abstract class TweetSet {
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def union(that: TweetSet): TweetSet = that.filterAcc(
    x => !this.contains(x), this)
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def descendingByRetweet: TweetList = if(!this.empty) new Cons (this mostRetweeted, this remove this.mostRetweeted descendingByRetweet) else Nil
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    if(!left.empty & !right.empty) max(elem, max (left.mostRetweeted, right.mostRetweeted))
    else if(!left.empty) max(elem, left.mostRetweeted)
    else if(!right.empty) max(elem, right.mostRetweeted)
    else elem
  }
